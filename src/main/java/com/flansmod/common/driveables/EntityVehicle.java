...
public class EntityVehicle extends EntityDriveable implements IExplodeable
{
    ...
    /*public float fr = (float)Math.random();
    public float fg = (float)Math.random();
    public float fb = (float)Math.random();*/
    
    //public String disk_path="";
    //public String str3 ="statusdisk_5rect";
    ...
    public IModelCustom d33kuzov;
    public double[] angle= new double[7];
    //TODO
    /*public float[] speeds;
    public boolean isKpp;
    public boolean isAKpp;
    public int numSpeed;*/
	...
	public EntityVehicle(World world, double x, double y, double z, EntityPlayer placer, VehicleType type, DriveableData data)
	{
		...
		/*data.fr = (float)Math.random();
		data.fg = (float)Math.random();
		data.fb = (float)Math.random();*/
		initType(type, false);
	}
	...
	public void readSpawnData(ByteBuf data)
	{
		...
		fg = data.readFloat();
		fb = data.readFloat();*/
		//disk_path = ByteBufUtils.readUTF8String(data);
		//disk_texture_path = ByteBufUtils.readUTF8String(data);
	}

	@Override
	public void writeSpawnData(ByteBuf data)
	{
		DriveableData data1 = getDriveableData();
		super.writeSpawnData(data);
		/*data.writeFloat(fr);
		data.writeFloat(fg);
		data.writeFloat(fb);*/
		//ByteBufUtils.writeUTF8String(data, disk_path);
		//ByteBufUtils.writeUTF8String(data, disk_texture_path);
	}
	...
	{
    	//if(KeyInputHandler.redminus.isPressed())System.out.println(211);
    	VehicleType type = getVehicleType();
		...
		switch(key)
		{
            ...
            {
            	throttle *= 0.7F;
            	if((throttle<=0.01F && throttle>0F)||(throttle>=-0.01F && throttle<0F))throttle=0F;
            	return true;
            } 
            //TODO
            /*case 25 : 
            {
            	if(numSpeed>0)numSpeed--;
            	return true;
            } 
            case 26 : 
            {
            	if(numSpeed<speeds.length)numSpeed++;
            	return true;
            } 
            case 27 : 
            {
	...
	public void onUpdate()
    {
       ...
       // str3=data.str1;
		if (disk_path != data.str1) {
			if (data.str1 != "") {
				subStr = data.str1.split(delimeter);
				if (subStr.length == 2) {
					disk_texture_path = subStr[0];
        ...
        if(disk_texture!= data.disk_texture)
        disk_texture = data.disk_texture;
        if(disk_model!= data.disk_model)
        disk_model = data.disk_model;
        
        if(kuzov_texture!= data.kuzov_texture)
        kuzov_texture = data.kuzov_texture;
        if(kuzov_model!= data.kuzov_model)
        kuzov_model = data.kuzov_model;
		...
		if(wheelsYaw < -20)
			wheelsYaw = -20;
		if(data.fr>1F) data.fr =1F;
		if(data.fr<0F) data.fr =0F;
		
		if(data.fg>1F) data.fg =1F;
		if(data.fg<0F) data.fg =0F;
		
		if(data.fb>1F) data.fb =1F;
		if(data.fb<0F) data.fb =0F;
		...
		//Sounds
		//Starting sound
		if (((throttle > 0.01F && throttle < 0.2F)||(throttle < -0.01F && throttle > -0.2F)) && soundPosition == 0 && hasEnoughFuel())
		{
			PacketPlaySound.sendSoundPacket(posX, posY, posZ, 50, dimension, type.startSound, false);
			soundPosition = type.startSoundLength;
		}
		//Flying sound
		if ((throttle > 0.2F || throttle < -0.2F) && soundPosition == 0 && hasEnoughFuel())
		{
