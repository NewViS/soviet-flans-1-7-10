...
public class GuiDriveableMenu extends GuiContainer
{
	...
	public void initGui()
	{
		...
		GuiButton tuningButton = new GuiButton(6, width / 2 +2, height / 2 -93, 58, 20, "Tuning");
		//bombButton.enabled = type.numBombSlots > 0;
		buttonList.add(tuningButton);
	}
	...
	protected void actionPerformed(GuiButton button)
    {
        ...
        if(button.id == 6) //Tuning
        {
        	FlansMod.getPacketHandler().sendToServer(new PacketDriveableGUI(6));
        	//inventory.player.openGui(FlansMod.INSTANCE, 7, world, entity.chunkCoordX, entity.chunkCoordY, entity.chunkCoordZ);
        }
