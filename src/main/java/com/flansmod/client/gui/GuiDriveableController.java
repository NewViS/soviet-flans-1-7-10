...
public class GuiDriveableController extends GuiScreen 
{
	...
	public void handleInput()
    {
		...
		if(plane != null && !plane.isDead() && plane.getControllingEntity() != null && plane.getControllingEntity() instanceof EntityPlayer)
		{
			...
			if(FlansMod.proxy.keyDown(KeyInputHandler.redplus.getKeyCode()))
			{
				plane.pressKey(18, player);
			}		
			if(FlansMod.proxy.keyDown(KeyInputHandler.redminus.getKeyCode()))
			{
				plane.pressKey(19, player);
			}		
			
			if(FlansMod.proxy.keyDown(KeyInputHandler.greenplus.getKeyCode()))
			{
				plane.pressKey(20, player);
			}		
			if(FlansMod.proxy.keyDown(KeyInputHandler.greenminus.getKeyCode()))
			{
				plane.pressKey(21, player);
			}	
				if(FlansMod.proxy.keyDown(KeyInputHandler.blueplus.getKeyCode()))
				{
					plane.pressKey(22, player);
				}		
				if(FlansMod.proxy.keyDown(KeyInputHandler.blueminus.getKeyCode()))
				{
				...
				if(FlansMod.proxy.keyDown(KeyInputHandler.keyBrake.getKeyCode())||FlansMod.proxy.keyDown(mc.gameSettings.keyBindJump.getKeyCode()))
				{
					plane.pressKey(24, player);
				}
				
				/*if(FlansMod.proxy.keyDown(KeyInputHandler.gearDown.getKeyCode()))
				{
					plane.pressKey(25, player);
				}
				if(FlansMod.proxy.keyDown(KeyInputHandler.gearUp.getKeyCode()))
				{
					plane.pressKey(26, player);
				}
				if(FlansMod.proxy.keyDown(KeyInputHandler.gearMode.getKeyCode()))
				{
